...
/**
 * @file drv8701.hpp
 * @brief DRV8701 直流有刷电机栅极驱动芯片（Head-only）。
 *
 * @details
 * - DRV8701 是 TI 的 H 桥栅极驱动器（外接 N-MOSFET），支持 PH/EN 和 PWM 两种控制模式。
 * - 本驱动不包含 PWM/PH 控制逻辑（EN/IN1、PH/IN2），仅封装芯片级的使能/睡眠与故障检测。
 ...
 * - 从睡眠模式唤醒后需等待 tWAKE（约 1ms）才能正常接收 PWM/PH 输入。
 */
class drv8701
{
public:
    /**
     * @brief DRV8701 故障状态结构体。
     *
     * @details
    ...
    /**
     * @brief 构造 DRV8701 驱动对象（仅绑定资源，不做外设初始化）。
     *
     * @param nsleep_gpio nSLEEP 引脚配置（高=使能，低=睡眠，内部下拉）
     * @param nfault_gpio nFAULT 引脚配置（开漏输出，低=故障，外部上拉）
     */
    drv8701(hal_gpio& nsleep_gpio, hal_gpio& nfault_gpio) noexcept
    ...
    drv8701(const drv8701&) = delete;
    drv8701& operator=(const drv8701&) = delete;
    drv8701(drv8701&&) = delete;
    drv8701& operator=(drv8701&&) = delete;
     ...
     *
     * @note
     * - 从睡眠模式唤醒后需等待 tWAKE（约 1ms）才能正常接收 EN/PH PWM 输入。
